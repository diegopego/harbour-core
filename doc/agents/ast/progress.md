...
- **Fixtures**: `tests/ast/ast_compilebuf_tests.c` gained a `-m` execution path that asserts trace capture from `hb_compMainExtModule()` remains intact while single-module mode is active. `tests/ast/ast_hbmk_ast_tests.c` now shares the capture harness, iterates over `fixture_demo.prg`, `fixture_blocks.prg`, and the new preprocessor-focused `fixture_ppdirectives.prg`, and validates the CLI dumps against the corresponding JSON fixtures.
