...
- **Fixtures**: `tests/ast/ast_compilebuf_tests.c` gained a `-m` execution path that asserts trace capture from `hb_compMainExtModule()` remains intact while single-module mode is active. `tests/ast/ast_hbmk_ast_tests.c` now shares the capture harness, iterates over `fixture_demo.prg` and `fixture_blocks.prg`, and validates the CLI dumps against `tests/ast/fixtures/fixture_demo.ast.json` / `fixture_blocks.ast.json`.
