...
- **Docs refreshed**: `Agents.md`, `doc/agents/ast/serialization-format.md`, and `README-AST.md` now explicitly call out the retired `src/ast/` / `utils/hbrename` overlay and direct consumers to the compiler-backed flow.
- **Cleanup phasing**:
...
- **Docs**: Noted in the harness documentation that CLI/compile-buffer executions must pass `-iinclude` so the standard Harbour headers resolve, recorded both compatibility fixtures in the verification matrix, and clarified that Harbour’s `ALWAYS` clause is the supported analogue to `FINALLY` (the compiler currently rejects `FINALLY`/`RETRY` under these pragma mixes).
...
- **Testing**: `bin/linux/gcc/hbmk2 -w3 tests/ast/fixture_blocks.prg`; `make -C tests/ast compilebuf-tests`; `./tests/ast/compilebuf-tests` (verifies both default and `-m` runs); `make -C tests/ast hbmk-ast-tests`; `./tests/ast/hbmk-ast-tests`; `make -C tests/ast trace-events`; `./tests/ast/trace-events`; `make -C tests/ast hbmk2-fixtures`; `./tests/ast/hbmk2-fixtures`. All tests pass.
- **Fixtures**: `tests/ast/ast_compilebuf_tests.c` gained a `-m` execution path that asserts trace capture from `hb_compMainExtModule()` remains intact while single-module mode is active. `tests/ast/ast_hbmk_ast_tests.c` now shares the capture harness, iterating over `fixture_demo.prg`, `fixture_blocks.prg`, the preprocessor-focused `fixture_ppdirectives.prg`, and the new declaration-statements fixture `fixture_statements.prg`, validating each against its JSON snapshot.
- **Diagnostics**: `tests/ast/ast_trace_tests.c` exercises the new diagnostics counters (token/boundary/node totals, traceinfo retain/release) and CLI toggles to ensure instrumentation and diagnostics can be controlled independently.
...
- **Verification**: `tests/ast/hbmk-ast-tests`, `./tests/ast/compilebuf-tests`, `./tests/ast/hbmk2-fixtures`, and `./scripts/test-ast.sh` all pass against the frozen pack; `bin/linux/gcc/hbmk2 -w3 tests/ast/*.prg` reports zero warnings.
- **Next steps**: Extend compile-buffer snapshot coverage before promoting the archive downstream.
