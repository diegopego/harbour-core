...
- Testes automatizados: `tests/ast/snapshot` constrói um lexer real, verifica o literal expandido por macro, garante que o snapshot exponha exatamente um grafo de expansão com ranges esperados e valida o JSON exportado por `hb_astTokenStreamSerializeMacrosJson()`/`hb_astTokenStreamWriteMacrosJson()`.
